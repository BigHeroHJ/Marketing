...
@interface MyTabBarController ()
{
    ...
    UIImageView   *_bottomView;//底部按钮视图
...
- (NSArray *)taBBarItemImage
{
    if (_taBBarItemImage == nil) {
        _taBBarItemImage = @[@"1.png",@"1.png",@"1.png",@"1.png"];
    }
...
- (NSArray *)taBBarItemSelectedImage
{
    if (_taBBarItemSelectedImage == nil) {
        _taBBarItemSelectedImage = @[@"1.png",@"1.png",@"1.png",@"1.png"];
    }
...
- (void)addTabBarView
{
    ...
    _bottomView  = [[UIImageView alloc] initWithFrame:CGRectMake(0, KSCreenH - 49, KSCreenW, 49)];
...
//    NSLog(@"bottomFrame:%@",NSStringFromCGRect(_bottomView.frame));
    ...
    for (int i = 0; i < self.tabBarItemTitle.count; i ++) {
        TabBarButton *btn = [[TabBarButton alloc] initWithFrame:CGRectMake(i * BtnW, 1, BtnW, 48)];
        ...
        btn.titleLabel.font = [UIView getFontWithFontName:@"Marion-Regular" Size:10.0f];
